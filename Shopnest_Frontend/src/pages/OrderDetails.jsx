...
function OrderDetails() {
  ...
  useEffect(() => {
    document.title = 'Order Details | ShopNest'
    if (token) {
      fetchOrderDetails()
    } else {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#070913] text-white flex flex-col items-center justify-center pt-20">
        ...
        <p className="text-slate-450 font-semibold uppercase tracking-wider text-sm">Loading invoice details...</p>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[#070913] text-white flex flex-col items-center justify-center px-4 pt-20">
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-8 max-w-md text-center backdrop-blur-xl">
          ...
          <p className="text-slate-450 mb-6 text-sm leading-relaxed">{error || "Order data missing"}</p>
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-6 gap-4">
          <div className="flex flex-col items-start gap-1">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight uppercase leading-none mt-2">
              Order <span className="bg-clip-text text-transparent bg-gradient-to-r from-violet-400 via-indigo-200 to-cyan-400">Ledger Details</span>
            </h1>
          </div>
          <p className="text-sm text-slate-500 max-w-sm leading-relaxed">
            Detailed information about your transaction logs, invoice allocations, and parcel transit.
          </p>
        </header>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md flex flex-wrap items-center justify-between gap-6">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-550 uppercase tracking-wider block">Shipment Status</span>
                <span className={`text-[10px] font-extrabold rounded-lg px-3 py-1 border uppercase tracking-wider ${
              ...
              <div className="flex gap-6">
                ...
                <div className="text-left">
                  <span className="text-[10px] font-bold text-slate-550 uppercase tracking-wider block mb-1">Est. Delivery</span>
                  <div className="flex items-center gap-1.5 text-xs text-indigo-400 font-semibold">
                    <Truck className="h-4 w-4 text-indigo-550" />
              ...
              <div className="space-y-3.5 text-slate-400 text-sm">
                ...
                <div className="border-t border-slate-800/60 pt-4 flex justify-between items-center text-xs">
                  ...
                  <span className="font-bold text-indigo-400 uppercase tracking-wider">
                    {order.paymentMethod === 'online' ? 'Online Gateway' : 'Cash on Delivery'}
                  </span>
                </div>
              </div>
            </div>
